...
from ppcls.arch.backbone.base.theseus_layer import EMAA
...
class RepVGG(nn.Layer):
    def __init__(self,
        ...
        ##加入几何先验和gsa
        # 插入 GSA 模块（假设只处理 stage2 和 stage3）
        # self.cross_gsa_modules = nn.LayerList([
        #     CrossGSA(embed_dim=320, num_heads=4),
        #     CrossGSA(embed_dim=256, num_heads=4)
        # ])
        self.cross_gsa_modules = nn.LayerList([
            CrossGSA(embed_dim=64, num_heads=4),    # stage0
            CrossGSA(embed_dim=160, num_heads=4),   # stage1
            CrossGSA(embed_dim=320, num_heads=4),   # stage2  # stage4
        ])

        self.out2_proj = nn.Conv2D(128, 320, kernel_size=1)

        # 几何先验生成模块
        self.geo_prior_gen = GeoPriorGen(embed_dim=256, num_heads=4, initial_value=4, heads_range=1)
        #self.EMAA =EMAA(64, factor=4)
        #self.EMAA = EMAA(40, factor=1) 
        self.EMAA = EMAA(160, factor=4)
    ...
    def forward(self, x, depth=None):
        ...
        #print(f'Input depth.shape: {depth.shape if depth is not None else "None"}')
        out = self.stage0(x)       
        out1 = self.stage1(out)
        # if depth is not None:
            

        #     depth = depth[:, 0:1, :, :]  # 取第0通道

            
        #     B, C1, H1, W1 = out1.shape
        ...
        #     feat1 = out1.transpose([0, 2, 3, 1])

        #     feat1 = self.cross_gsa_modules[1](feat1, rel_pos=geo1)

            
        #     out1 = feat1.transpose([0, 3, 1, 2])
        #     print("out1.shape before EMAA:", out1.shape)
