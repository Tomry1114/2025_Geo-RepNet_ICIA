...
from ppcls.arch.backbone.base.theseus_layer import GeoPriorGen
from ppcls.arch.backbone.base.theseus_layer import CrossGSA
...
class LePEAttention(nn.Layer):
    ...
    def im2cswin(self, x):
        ...
        #print(f"img shape before img2windows: {x.shape}")
        #print(f"h_split: {self.h_split}, w_split: {self.w_split}")
...
class CSwinBlock(nn.Layer):
    ...
    def __init__(self,
        ...
        self.cross_gsa_modules = CrossGSA(embed_dim=dim,num_heads=num_heads) 

        self.geo_prior_gen = GeoPriorGen(embed_dim=dim, num_heads=num_heads, initial_value=4, heads_range=1)
    ...
    def forward(self, x, depth=None):
        ...
        if depth is not None:
            #print("Using depth for geometric prior generation")
            # Step 1: reshape x -> [B, C, H, W]
            x_reshaped = x.transpose([0, 2, 1]).reshape([B, C, H, W])
            H, W = x_reshaped.shape[2], x_reshaped.shape[3]
            H_Ori, W_ori = H, W  # 保存原始 H 和 W
            #print(f"Before downsampling, x_reshaped shape: {x_reshaped.shape}, H: {H}, W: {W}")

            # 下采样，减小H和W
            x_reshaped = paddle.nn.functional.avg_pool2d(x_reshaped, kernel_size=2, stride=2)
            H, W = x_reshaped.shape[2], x_reshaped.shape[3]
            print(f"After downsampling, x_reshaped shape: {x_reshaped.shape}, H: {H}, W: {W}")

            # 对 depth 做同样下采样，保证匹配
            depth = depth[:, 0:1, :, :]
            depth = paddle.nn.functional.avg_pool2d(depth, kernel_size=2, stride=2)
            #print(f"Downsampled depth shape: {depth.shape}")

            # Step 3: 几何先验生成
            geo1 = self.geo_prior_gen((H, W), depth)
            (sin, cos), mask = geo1

            # Step 4: GSA 融合
            feat = x_reshaped.transpose([0, 2, 3, 1])  # [B, H, W, C]
            feat = self.cross_gsa_modules(feat, rel_pos=geo1)
            x_reshaped = feat.transpose([0, 3, 1, 2])  # [B, C, H, W]

            x_upsampled = F.interpolate(x_reshaped, size=(H_Ori, W_ori), mode='bilinear', align_corners=False)

            # Step 5: reshape 回 transformer 格式
            x = x_upsampled.reshape([B, C, H_Ori * W_ori]).transpose([0, 2, 1])
...
class MergeBlock(nn.Layer):
    ...
    def forward(self, x):
        ...
        H = W = int(np.sqrt(HW))
        #print(f"Input shape: {x.shape}, H: {H}, W: {W}, B: {B}, C: {C}")
...
class CSwinTransformer(nn.Layer):
    ...
    # def forward_features(self, x,depth=None):
    ...
    def forward(self, x, depth=None):
        x = self.forward_features(x, depth)  # 这里也传递 depth
        x = self.head(x)
        return x
