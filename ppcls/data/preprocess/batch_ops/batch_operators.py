...
class BatchOperator(object):
    ...
    def _unpack(self, batch):
        assert isinstance(batch, list), 'batch should be a list filled with tuples (img, depth_img, label)'
        bs = len(batch)
        assert bs > 0, 'size of the batch data should > 0'
        imgs = []
        depth_imgs = []
        ...
        for item in batch:
            imgs.append(item[0])
            depth_imgs.append(item[1])
            labels.append(item[2])
        return np.array(imgs), np.array(depth_imgs), np.array(labels), bs


    def _one_hot(self, targets):
        #print(f"targets unique values: {np.unique(targets)}")
        #print("targets type:", type(targets), "dtype:", getattr(targets, 'dtype', None))
        targets = np.array(targets).astype(np.int64)  # 确保是整数类型
...
class MixupOperator(BatchOperator):
    ...
    def __call__(self, batch):
        imgs, depth_imgs, labels, bs = self._unpack(batch)
        idx = np.random.permutation(bs)
        lam = np.random.beta(self._alpha, self._alpha)

        imgs = lam * imgs + (1 - lam) * imgs[idx]
        #depth_imgs = lam * depth_imgs + (1 - lam) * depth_imgs[idx]
        depth_imgs = depth_imgs 

        targets = self._mix_target(labels, labels[idx], lam)
        # 返回时，把 imgs, depth_imgs, targets 对应配对返回
        return list(zip(imgs, depth_imgs, targets))
