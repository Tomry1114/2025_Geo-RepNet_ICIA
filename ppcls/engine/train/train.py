...
def train_epoch(engine, epoch_id, print_batch_step):
    ...
    for iter_id in range(engine.iter_per_epoch):
        ...
        if not engine.config["Global"].get("use_multilabel", False):
            batch[2] = batch[2].reshape([batch_size, -1])
        engine.global_step += 1

        # image input
        with engine.auto_cast(is_eval=False):
                        # ✅ 检查输入图像是否含有 NaN（建议只打印前几个）
            img = batch[0]  # RGB，形状 [B, 3, H, W]
            depth = batch[1]  # Depth，形状 [B, 3, H, W] 但理论上应是 1 通道

            # print("RGB first channel sample stats:")
            # print("min:", img[0, 0].min().item(), "max:", img[0, 0].max().item())
            # print("mean:", img[0, 0].mean().item())

            # print("Depth first channel sample stats:")
            # print("min:", depth[0, 0].min().item(), "max:", depth[0, 0].max().item())
            # print("mean:", depth[0, 0].mean().item())
            ...
            loss_dict = engine.train_loss_func(out, batch[2])
...
def forward(engine, batch):
    if not engine.is_rec:
        ...
        return engine.model(batch[0], batch[2])
