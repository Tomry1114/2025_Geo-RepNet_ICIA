...
class AucScore(AvgMetrics):
    ...
    def forward(self, x, label):
        ...
        try:
            ...
            if num_classes == 2:
                auc = roc_auc_score(y_true_onehot, y_pred, average=self.average)
            else:
                auc = roc_auc_score(y_true_onehot, y_pred, multi_class='ovr', average=self.average)
        ...
        self.avg_meters["auc"].update(auc, x.shape[0])
        return {"auc": auc}



class F1Score(AvgMetrics):
    def __init__(self, average="weighted"):
        """
        Args:
    ...
    def forward(self, x, label):
        ...
        self.y_pred.extend(pred.tolist())
        self.y_true.extend(label.tolist())

        try:
            score = f1_score(self.y_true, self.y_pred, average=self.average)
        except Exception as e:
            print(f"[Warning] F1 computation failed: {e}")
            score = 0.0

        self.avg_meters["f1"].update(score, x.shape[0])
        return {"f1": score}
